...
try:
    with codecs.open('diff_utf8.txt', 'r', 'utf8', errors='ignore') as f:
        lines = f.readlines()
...
md.append(f'**追加された物件**: {added_count} 件\n')
md.append(f'**削除された（条件外になった）物件**: {removed_count} 件\n\n')

md.append('## エリアごとの増減\n')
md.append('| エリア | 追加 | 削除 | 増減 |\n')
md.append('|---|---|---|---|\n')
