 ...
 * 
 * 目的:
 *   指定された外部CSV（例: .work/d2d30/station_commute_extra.csv）の未取得行（station_to_office_min が空）のみを対象に、
...
async function runExtra() {
    ...
    if (!csvPath) {
        console.error('エラー: --csv オプションで対象CSVパスを指定してください。 (例: --csv .work/d2d30/station_commute_extra.csv)');
        process.exit(1);
    }
    ...
    const targetTotal = fetchedCount + failedCount;
    if (targetTotal > 0 && (failedCount / targetTotal) > 0.2) {
        console.warn(`\n【警告】失敗率が20%を超えています (${failedCount}/${targetTotal})。確認が必要です。`);
    }
}
