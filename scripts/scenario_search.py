...
def modify_url_params(base_url: str, **kwargs: Union[str, int, float]) -> str:
    """URL のクエリパラメータを安全に差し替えて再構成する"""
...
def stage_selfcheck(ps_mod: ModuleType, js_path: str = BASE_PROPERTIES_JS_PATH, csv_path: str = BASE_STATION_COMMUTE_CSV_PATH) -> bool:
    ...
    1. calculate_self_pay が保存済み self_pay と一致すること。
    2. evaluate_best_commute が保存済み door_to_door および total_walk_min と一致すること。
    """
...
def stage_fetch(ps_mod: ModuleType, work_dir: str, tj: int, min_area: int, max_rent: float, max_walk: int, refresh: bool) -> bool:
    """
    Stage: fetch (SUUMO一覧の取得)
    5都県から指定条件の一覧ページを順次取得し、raw_listings.json に保存する。
    """
    print(f"=== Stage: fetch（SUUMO一覧の取得 tj={tj}, mb={min_area}, ct={max_rent}, et={max_walk}） ===")
    ...
    for pref_name, pref_code in PREFECTURES.items():
        ...
        target_url = modify_url_params(base_url, tj=tj)
...
def stage_stations(ps_mod: ModuleType, work_dir: str, max_walk: int, max_age: int, max_self_pay: float) -> bool:
    """
    Stage: stations (通勤時間を取得する新規駅の洗い出し)
    raw_listings.json から重複排除・基本フィルタを適用し、
    不足駅を station_commute_extra.csv に空行として書き出す。
    """
    ...
    registered_stations: Set[str] = set(base_station_db.keys()) | existing_extra_stations
    print(f"既存登録済み駅数: 本番DB={len(base_station_db)} 駅, extraDB={len(existing_extra_stations)} 駅")

    # 3. 候補駅の抽出
    new_stations_dict: Dict[str, str] = {}  # station_name -> primary_line
    ...
    print(f"基本条件（駅徒歩{max_walk}分以下・築{max_age}年以下・最低自己負担{max_self_pay}万以下）通過物件数: {passed_filter_props} 件")
    print(f"新規追加対象駅数: {len(new_stations_dict)} 駅")

    # 4. station_commute_extra.csv の書き出し
...
def stage_parking(ps_mod: ModuleType, work_dir: str, d2d_bands_str: str, max_total_walk: int, max_self_pay: float) -> bool:
    """
    Stage: parking (対象物件の駐車場代取得)
    新通勤DBをマージしてドアドアを評価し、帯の最大値以下かつ総徒歩上限以下の物件を対象に
    駐車場代キャッシュを作成・更新する。
    """
    ...
    print(f"対象帯上限: {max_d2d_target} 分以下, 総徒歩上限: {max_total_walk} 分以下")

    # 1. 通勤DBのマージ
    base_station_db = ps_mod.load_station_commute_db(BASE_STATION_COMMUTE_CSV_PATH)
    extra_station_db = ps_mod.load_station_commute_db(extra_csv_path) if os.path.exists(extra_csv_path) else {}
    # 同名駅は本番優先
    merged_station_db = dict(extra_station_db)
    merged_station_db.update(base_station_db)
    print(f"マージ後通勤DB駅数: {len(merged_station_db)} 駅 (本番:{len(base_station_db)}, extra:{len(extra_station_db)})")

    # 2. 駐車場キャッシュのマージ（本番は読み取りのみ）
    ...
    for idx, p in enumerate(target_listings):
        ...
        if url in base_parking_cache:
            continue
        if url in extra_parking_cache:
            continue
...
def stage_report(ps_mod: ModuleType, work_dir: str, d2d_bands_str: str, max_total_walk: int, max_self_pay: float) -> bool:
    """
    Stage: report (集計・テーブル生成)
    候補物件の同一物件統合を行い、帯別（30分, 35分, 40分, ベースライン）の集計表および
    物件一覧テーブルを生成する。
    """
    ...
    # 6. 集計計算
    # ベースラインデータの集計
    with open(BASE_PROPERTIES_JS_PATH, "r", encoding="utf-8") as f:
        base_raw = f.read()
    m_base = re.search(r"const\s+bukkenData\s*=\s*(.*);", base_raw, re.DOTALL)
    base_items: List[Dict[str, Union[str, float, int, None]]] = json.loads(m_base.group(1).strip()) if m_base else []

    def compute_stats(items: List[Dict[str, Union[str, float, int, bool, None]]], is_base: bool = False) -> Dict[str, Union[int, float, str]]:
        if not items:
            return {
                "count": 0, "area_min": 0, "area_med": 0, "area_max": 0,
                "age_min": 0, "age_med": 0, "age_max": 0,
                "bed3_ratio": 0.0, "match_baseline_cnt": 0, "floor3_ratio": 0.0,
        ...
        bed3_count = sum(1 for x in items if int(x.get("bedrooms", 0) or 0) >= 3 or extract_bedrooms(str(x.get("madori", ""))) >= 3)
        floor3_count = sum(1 for x in items if "3階建" in str(x.get("age_floor", "")))

        # 現行条件（80m2以上かつ築30年以下）を満たす件数
        match_baseline_cnt = 0
        for x in items:
            m_a = re.search(r"[\d.]+", str(x.get("menseki", "")))
            area_val = float(m_a.group(0)) if m_a else 0.0
            age_val = ps_mod.parse_age_num(str(x.get("age_floor", "")))
            if area_val >= 80.0 and age_val <= 30:
                match_baseline_cnt += 1
        ...
        return {
            ...
            "bed3_ratio": round(bed3_count / len(items) * 100, 1),
            "match_baseline_cnt": match_baseline_cnt,
            "floor3_ratio": round(floor3_count / len(items) * 100, 1),
    ...
    base_stats = compute_stats(base_items, is_base=True)

    # 7. tables.md の構築
    md_lines: List[str] = []
    md_lines.append("# 集計テーブル（ドアドア30分シナリオ）\n")
    md_lines.append("## 1. 帯別集計表（累積）\n")
    md_lines.append("| 指標 | ドアドア30分以下 | ドアドア35分以下 | ドアドア40分以下 | ベースライン（現行） |")
    md_lines.append("| :--- | :---: | :---: | :---: | :---: |")
    ...
    def format_stat_row(label: str, key_min: str, key_med: str, key_max: str, unit: str = "") -> str:
        ...
        s_base = f"{base_stats[key_min]} / {base_stats[key_med]} / {base_stats[key_max]}"
        return f"| {label} | {s30}{unit} | {s35}{unit} | {s40}{unit} | {s_base}{unit} |"

    md_lines.append(f"| 件数（統合後） | **{band_stats[30]['count']}件** | **{band_stats[35]['count']}件** | **{band_stats[40]['count']}件** | **{base_stats['count']}件** |")
    ...
    md_lines.append(f"| 居室3以上の割合 | {band_stats[30]['bed3_ratio']}% | {band_stats[35]['bed3_ratio']}% | {band_stats[40]['bed3_ratio']}% | {base_stats['bed3_ratio']}% |")
    md_lines.append(f"| 現行条件も満たす件数（80m²以上・築30年以下） | {band_stats[30]['match_baseline_cnt']}件 | {band_stats[35]['match_baseline_cnt']}件 | {band_stats[40]['match_baseline_cnt']}件 | {base_stats['count']}件 |")
    md_lines.append(f"| 3階建ての割合 | {band_stats[30]['floor3_ratio']}% | {band_stats[35]['floor3_ratio']}% | {band_stats[40]['floor3_ratio']}% | {base_stats['floor3_ratio']}% |")
    md_lines.append(f"| 自己負担 中央値 | {band_stats[30]['self_pay_med']}万円 | {band_stats[35]['self_pay_med']}万円 | {band_stats[40]['self_pay_med']}万円 | {base_stats['self_pay_med']}万円 |")
    md_lines.append(f"| 駅徒歩 中央値 | {band_stats[30]['walk_med']}分 | {band_stats[35]['walk_med']}分 | {band_stats[40]['walk_med']}分 | {base_stats['walk_med']}分 |")
    md_lines.append(f"| 駐車場（敷地内 / 近隣 / 記載なし） | {band_stats[30]['park_site']} / {band_stats[30]['park_near']} / {band_stats[30]['park_unknown']} | {band_stats[35]['park_site']} / {band_stats[35]['park_near']} / {band_stats[35]['park_unknown']} | {band_stats[40]['park_site']} / {band_stats[40]['park_near']} / {band_stats[40]['park_unknown']} | {base_stats['park_site']} / {base_stats['park_near']} / {base_stats['park_unknown']} |")
    md_lines.append("")

    # 妥協量まとめ
    if band_stats[30]["count"] > 0:
        area_diff = round(float(band_stats[30]["area_med"]) - float(base_stats["area_med"]), 2)
        age_diff = round(float(band_stats[30]["age_med"]) - float(base_stats["age_med"]), 1)
        bed3_diff = round(float(band_stats[30]["bed3_ratio"]) - float(base_stats["bed3_ratio"]), 1)
        md_lines.append(f"**ベースラインとの中央値差（ドアドア30分以下）:**")
        md_lines.append(f"- 専有面積: {area_diff:+.2f} m²（{base_stats['area_med']} m² → {band_stats[30]['area_med']} m²）")
        md_lines.append(f"- 築年数: {age_diff:+.1f} 年（{base_stats['age_med']} 年 → {band_stats[30]['age_med']} 年）")
        md_lines.append(f"- 居室3以上の割合: {bed3_diff:+.1f} pt（{base_stats['bed3_ratio']}% → {band_stats[30]['bed3_ratio']}%）")
        md_lines.append("")

    # 推定ルート物件（参考）
    if estimated_items:
        md_lines.append(f"### （参考）推定ルート採用のため件数外とした物件: {len(estimated_items)}件\n")
        md_lines.append("| 駅 | 路線 | 物件名 | 自己負担 | 推定ドアドア | 間取り/面積 | 築年数 | リンク |")
        md_lines.append("| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :--- |")
        for x in estimated_items:
            md_lines.append(f"| {x['station']} | {x['line']} | {x['title']} | {x['self_pay']}万 | {x['door_to_door']}分 | {x['madori']} / {x['menseki']} | {x['age_floor']} | [詳細]({x['url']}) |")
        md_lines.append("")
    ...
    if d2d30_items:
        md_lines.append("| 駅 | 路線 | 物件名 | 家賃/管理費 | 自己負担 | ドアドア (徒歩+乗車) | 間取り/面積 | 築年数 | 駐車場 | リンク |")
        md_lines.append("| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- |")
        for x in sorted(d2d30_items, key=lambda it: (int(it.get("door_to_door", 0) or 0), float(it.get("self_pay", 0.0) or 0.0))):
            ...
            md_lines.append(f"| {x['station']} | {x['line']} | {x['title']} | {rent_adm} | {x['self_pay']}万 | {d2d_breakdown} | {madori_menseki} | {x['age_floor']} | {x['parking_text']} | [詳細]({x['url']}) |")
    else:
        md_lines.append("該当物件はありません。\n")
    md_lines.append("")

    # 駅別件数（40分以下・30分以下）
    md_lines.append("## 3. 駅別の物件件数（ドアドア40分以下・30分以下）\n")
    station_counts_40: Dict[str, Dict[str, Union[str, int]]] = {}
    for x in confirmed_items:
        st = str(x.get("station", "不明"))
        ln = str(x.get("line", "不明"))
        d = int(x.get("door_to_door", 0) or 0)
        if st not in station_counts_40:
            station_counts_40[st] = {"line": ln, "cnt_30": 0, "cnt_35": 0, "cnt_40": 0}
        if d <= 30:
            station_counts_40[st]["cnt_30"] = int(station_counts_40[st]["cnt_30"]) + 1
        if d <= 35:
            station_counts_40[st]["cnt_35"] = int(station_counts_40[st]["cnt_35"]) + 1
        if d <= 40:
            station_counts_40[st]["cnt_40"] = int(station_counts_40[st]["cnt_40"]) + 1

    md_lines.append("| 駅名 | 主要路線 | 30分以下 | 35分以下 | 40分以下 |")
    md_lines.append("| :--- | :--- | :---: | :---: | :---: |")
    for st, c in sorted(station_counts_40.items(), key=lambda it: (-it[1]["cnt_40"], -it[1]["cnt_30"], it[0])):
        md_lines.append(f"| {st} | {c['line']} | {c['cnt_30']}件 | {c['cnt_35']}件 | {c['cnt_40']}件 |")
    md_lines.append("")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ドアドア30分シナリオ 調査スクリプト")
    parser.add_argument("--stage", choices=["selfcheck", "fetch", "stations", "parking", "report"], required=True,
                        help="実行ステージ: selfcheck (一致確認), fetch (SUUMO取得), stations (駅洗い出し), parking (駐車場取得), report (集計)")
    ...
    if args.stage == "selfcheck":
        ...
        ok = stage_fetch(ps_mod, args.work_dir, args.tj, args.min_area, args.max_rent, args.max_walk, args.refresh)
    elif args.stage == "stations":
        ok = stage_stations(ps_mod, args.work_dir, args.max_walk, args.max_age, args.max_self_pay)
    elif args.stage == "parking":
        ok = stage_parking(ps_mod, args.work_dir, args.d2d_bands, args.max_total_walk, args.max_self_pay)
    elif args.stage == "report":
        ok = stage_report(ps_mod, args.work_dir, args.d2d_bands, args.max_total_walk, args.max_self_pay)
    else:
        ok = False
