...
【注意: Shapefile使用時の文字化けについて】
KSJ配布zip内の「UTF-8」フォルダのShapefileであっても、.cpg(エンコーディング指定)
ファイルが同梱されておらず、環境によってはpyogrio/GDALがDBF内の日本語属性を
誤ったエンコーディングで読み込み、路線名・事業者名が文字化けして一切マッチしなく
なることがある(実際に2025年度版で確認済み)。同梱の.geojsonファイル
(例: N02-25_RailroadSection.geojson)は文字化けが起きないため、Shapefileで
路線名が文字化けする場合は同名の.geojsonを--inputに指定して回避すること。
...
MANUAL_LINE_NAME_MAP = {
    ...
    "小田急線": ["小田原線"],  # KSJ側は正式路線名「小田原線」で登録されている(江ノ島線・多摩線とは別区間)
    "東武東上線": ["東上本線"],  # KSJ側は「東上本線」表記
    # ＪＲ埼京線: KSJ上は赤羽線/東北線/川越線の一部として登録されており独立した路線名が無いため、
    # 対応関係を追加しても正しい区間を抽出できない。未マッチのまま直線描画にフォールバックさせる。
}
